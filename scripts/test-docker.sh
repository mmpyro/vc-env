...
# ── Step 4: vc-env install {version} ──────────────────────────────────────────
TEST_VERSION="0.21.1"
section "Step 4: vc-env install ${TEST_VERSION}"
INSTALL_OUTPUT=$(vc-env install "${TEST_VERSION}" 2>&1)
INSTALL_EXIT=$?
if [ $INSTALL_EXIT -eq 0 ] && [ -f "${VCENV_ROOT}/versions/${TEST_VERSION}/vcluster" ]; then
    pass 4 "vc-env install ${TEST_VERSION} succeeded; binary present at versions/${TEST_VERSION}/vcluster"
else
    fail 4 "vc-env install ${TEST_VERSION}" \
...
# ── Step 6: vc-env global {version} ───────────────────────────────────────────
section "Step 6: vc-env global ${TEST_VERSION}"
vc-env global "${TEST_VERSION}" 2>&1
GLOBAL_READ=$(vc-env global 2>&1)
GLOBAL_EXIT=$?
if [ $GLOBAL_EXIT -eq 0 ] && echo "${GLOBAL_READ}" | grep -q "${TEST_VERSION}"; then
    pass 6 "vc-env global set and confirmed as '${TEST_VERSION}'"
else
    fail 6 "vc-env global ${TEST_VERSION}" \
...
if [ $WHICH_EXIT -eq 0 ] && echo "${WHICH_OUTPUT}" | grep -q "${EXPECTED_WHICH}"; then
    pass 7 "vc-env which returns correct path '${EXPECTED_WHICH}'"
else
    fail 7 "vc-env which" \
         "vc-env which" \
...
# ── Step 8: vc-env local {version} ────────────────────────────────────────────
section "Step 8: vc-env local ${TEST_VERSION}"
...
if [ $LOCAL_EXIT -eq 0 ] \
   ...
   && grep -q "${TEST_VERSION}" "${LOCAL_WORKDIR}/.vcluster-version"; then
    pass 8 "vc-env local set '${TEST_VERSION}'; .vcluster-version file written correctly"
else
    fail 8 "vc-env local ${TEST_VERSION}" \
...
# ── Step 9: vc-env shell {version} ────────────────────────────────────────────
section "Step 9: vc-env shell ${TEST_VERSION}"
...
if [ $SHELL_EXIT -eq 0 ] && echo "${SHELL_OUTPUT}" | grep -q "export VCENV_VERSION=${TEST_VERSION}"; then
    pass 9 "vc-env shell outputs 'export VCENV_VERSION=${TEST_VERSION}'"
else
    fail 9 "vc-env shell ${TEST_VERSION}" \
         "vc-env shell ${TEST_VERSION}" \
         "output contains 'export VCENV_VERSION=${TEST_VERSION}' (exit 0)" \
         "exit=${SHELL_EXIT}; output=${SHELL_OUTPUT}"
fi

# ── Step 10: vc-env uninstall {version} ───────────────────────────────────────
section "Step 10: vc-env uninstall ${TEST_VERSION}"
...
if [ $UNINSTALL_EXIT -eq 0 ] \
   ...
   && [ ! -d "${VCENV_ROOT}/versions/${TEST_VERSION}" ]; then
    pass 10 "vc-env uninstall ${TEST_VERSION} succeeded; version directory removed"
else
    fail 10 "vc-env uninstall ${TEST_VERSION}" \
...
if ! echo "${LIST_AFTER}" | grep -q "${TEST_VERSION}"; then
    pass 10b "vc-env list no longer shows ${TEST_VERSION} after uninstall"
else
    fail 10b "vc-env list after uninstall" \
         "vc-env list" \
         "output does NOT contain '${TEST_VERSION}'" \
         "${LIST_AFTER}"
fi

# ── Step 11: vc-env version ───────────────────────────────────────────────────
section "Step 11: vc-env version"
VERSION_OUTPUT=$(vc-env version 2>&1)
VERSION_EXIT=$?
if [ $VERSION_EXIT -eq 0 ] && echo "${VERSION_OUTPUT}" | grep -qE '[0-9]+\.[0-9]+\.[0-9]+'; then
    pass 11 "vc-env version outputs a version string: '${VERSION_OUTPUT}'"
else
    fail 11 "vc-env version" \
         "vc-env version" \
...
# ── Step 12: vc-env help ──────────────────────────────────────────────────────
section "Step 12: vc-env help"
HELP_OUTPUT=$(vc-env help 2>&1)
HELP_EXIT=$?
if [ $HELP_EXIT -eq 0 ] && echo "${HELP_OUTPUT}" | grep -qi "Commands:"; then
    pass 12 "vc-env help outputs usage information containing 'Commands:'"
else
    fail 12 "vc-env help" \
         "vc-env help" \
...
# ── Step 13: shim end-to-end — reinstall then run vcluster via shim ───────────
section "Step 13: vcluster shim end-to-end"
...
if [ $REINSTALL_EXIT -ne 0 ]; then
    fail 13 "vcluster shim (re-install prerequisite)" \
         "vc-env install ${TEST_VERSION}" \
    ...
    if ! command -v vcluster >/dev/null 2>&1; then
        fail 13 "vcluster shim not found in PATH" \
             "command -v vcluster" \
        ...
        if echo "${VCLUSTER_OUTPUT}" | grep -qi "vcluster\|version\|[0-9]\+\.[0-9]"; then
            pass 13 "vcluster shim at '${SHIM_PATH}' executed correctly; output: $(echo "${VCLUSTER_OUTPUT}" | head -1)"
        else
            fail 13 "vcluster shim end-to-end" \
                 "vcluster version" \
